...
const {
    ...
    findRootTypeFieldForAugmentedTarget, applyOnRootTypeFieldByTypeMode,
...
class SchemaAugmenter extends SchemaDirectiveVisitor {
    ...
    visitFieldDefinition(field, details) {
        ...
        if (isRootType(details.objectType, this.schema)) {
            ...
            const existing = findRootTypeFieldForAugmentedTarget(this.schema, this.args.type, mode);
            if (existing) {
                throw new Error(
                    `directive "@${mode}" should be used on ` +
                    `only 1 root query / mutation field for type "${this.args.type}"`
                );
            }
